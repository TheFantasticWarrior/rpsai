#!/usr/bin/env python
# edited from rpsrunner.py from rpscontest.com
try:
...
class Contest:
    ...
    def __init__(self, bot1, bot2, rounds=100):
        self.bot1 = bot1
        self.bot2 = bot2
        self.rounds = rounds
...
def report_results(bots, results):
    ...
    win_ratio = [(x,float(matches_won[x])/matches_played[x]) for x in botnames]
    botnames=[i for i,_ in sorted(win_ratio,key=lambda x:x[1],reverse=True)]
    ...
    for bot in botnames:
        ...
        print(f"{bot}: won {float(matches_won[bot]) / matches_played[bot] * 100:.1f}% of matches ({matches_won[bot]} of {matches_played[bot]})\n\
                won {float(rounds_won[bot])/rounds_played[bot]*100:.1f}% of rounds ({rounds_won[bot]} of {rounds_played[bot]})\n\
                avg score {float(scores[bot])/ matches_played[bot]:.1f}, net score {scores[bot]:.1f}\n")
...
def usage(msg, exit=0):
    print(f"\nRock-Paper-Scissors Runner v{VERSION} (http://www.rpscontest.com/)\n\
...
\n\
   Options:\n\
     ...
     -t|--threads <N>  How many execution threads to use [def: {POOL_SIZE}]\n\
        ...
        execution thread.  On this host, multiprocessing is: {'AVAILABLE' if Pool is not None else 'UNAVAILABLE'}\n\
\n\
